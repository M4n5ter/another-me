...
type (
	// AMFetchTool 是一个用于获取网页内容的 Eino 工具。
	AMFetchTool struct {
		logger *slog.Logger
	}

	// AMFetchToolArgs 定义了 AMFetchTool 的参数。
	AMFetchToolArgs struct {
		...
		IsManualRequest bool   `json:"is_manual_request,omitempty"` // 指示请求是否由用户操作手动触发（影响默认 User-Agent 和 robots.txt 检查行为）
	}

	// AMFetchToolResult 定义了成功结果的结构。
	AMFetchToolResult struct {
		...
		NextStartIndex *int   `json:"next_start_index,omitempty"` // 如果内容被截断，下次获取应使用的 start_index (仅当 IsTruncated 为 true 时存在)
		Message        string `json:"message,omitempty"`          // 附加的状态信息（例如，关于内容类型或截断的警告）
		ContentType    string `json:"content_type,omitempty"`     // 响应的 Content-Type 头
	}
)

// NewAMFetchTool 创建一个新的 AMFetchTool 实例。
func NewAMFetchTool() *AMFetchTool {
	return &AMFetchTool{
		logger: slog.Default().WithGroup("am_fetch"),
	}
}

var _ tool.InvokableTool = (*AMFetchTool)(nil)
...
// InvokableRun 执行获取操作。
func (t *AMFetchTool) InvokableRun(ctx context.Context, argumentsInJSON string, _opts ...tool.Option) (string, error) {
	var args AMFetchToolArgs
	if err := json.Unmarshal([]byte(argumentsInJSON), &args); err != nil {
		t.logger.Error("解析参数失败", "error", err, "args", argumentsInJSON)
	...
	if err != nil {
		...
		// TODO: 明确检查上下文取消错误 (context.Canceled)
...
func (t *AMFetchTool) setupHTTPClient(proxyURLStr string) (*http.Client, error) {
...
func (t *AMFetchTool) checkRobotsTxt(ctx context.Context, client *http.Client, targetURL *url.URL, userAgent string) (bool, error) {
	...
	default:
		...
		// 或者根据策略返回错误: return false, fmt.Errorf("获取 robots.txt (%s) 时状态异常: %s", robotsURL, resp.Status)
